import { GAS_LIMITS } from '@/config/fees';
...
async function executeDirectSwap(params: MultihopSwapParams): Promise<SwapResult> {
  ...
  const tx = await params.client.tx.snip20.send(
    ...
    {
      gasLimit: GAS_LIMITS.DIRECT_SWAP,
    }
  );
...
async function executeRouterSwap(params: MultihopSwapParams): Promise<SwapResult> {
  ...
  // Calculate gas limit: base + per-hop allowance
  const estimatedGasLimit = Math.min(
    GAS_LIMITS.MAX_MULTIHOP,
    GAS_LIMITS.MULTIHOP_BASE + routerHops.length * GAS_LIMITS.MULTIHOP_PER_HOP
  );
